...
void TrajBuilder::build_braking_traj(geometry_msgs::PoseStamped start_pose, nav_msgs::Odometry current_state,
        std::vector<nav_msgs::Odometry> &vec_of_states) {
    ...
    double initial_speed = current_state.twist.twist.linear.x;
    double current_speed = initial_speed;
    double initial_spin_rate = current_state.twist.twist.angular.z;
    double current_spin_rate = initial_spin_rate;
    
    double des_speed;
    double des_spin_rate;
    ...
    while (current_speed > 0) {
        ...
        des_speed = current_speed - (accel_max_ * t);        
        des_state.twist.twist.linear.x = des_speed; //update speed
        
        //update positions
        x_des = x_start + ((initial_speed * t) - (0.5 * accel_max_ * t * t)) * cos(current_psi);
        y_des = y_start + ((initial_speed * t) - (0.5 * accel_max_ * t * t)) * sin(current_psi);
        
        current_speed = des_speed;
    ...
    // create one more state to make sure velocity is 0
